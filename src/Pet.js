...
const Pet = (props) => {
  ...
  return (
    <div className='pet'>
      <div>
        {photos.map((photo, index) => <img key={photo.value} src={photo.value} alt={`${props.pet.name} number ${index + 1}`} />)}
      </div>
      <ul>
        <li>{props.pet.name}</li>
        <li>
          {props.pet.animal} : {Array.isArray(props.pet.breeds.breed)
            ? props.pet.breeds.breed.join(', ') : props.pet.breeds.breed}
        </li>
        <li>{props.pet.age}</li>
